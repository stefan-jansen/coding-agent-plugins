...
|---|---|---|
| `workflow` | `/align` `/plan-issues` `/next-issue` `/ship` `/handoff` `/continue` `/delegate` `/explore` `/plan` `/next` `/work` `/spike` | Structured task execution — spec → issues → implement → deliver, plus session handoffs |
