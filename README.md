...
> - **Project-local storage** (`.workspace/work/` not global `~/.claude/plans/`)
...
**Work units** are organized as: `.workspace/work/YYYY-MM-DD_NN_description/`
...
# Creates: .workspace/transitions/YYYY-MM-DD/HHMMSS.md
...
continue
# Or explicitly:
continue from .workspace/transitions/2026-05-08/223452.md
```

**Transition files become project history**: Over time, `.workspace/transitions/` becomes a chronological record of project evolution—far more useful than generic commit messages.

**`/index [--update] [focus_area]`** - Create persistent project knowledge base
```bash
/index
# Analyzes: Project structure, patterns, conventions
# Generates: Memory files in .workspace/memory/ loaded via @import from AGENTS.md
```
...
**Solution**: Ensure project has `.workspace/` directory:
```bash
