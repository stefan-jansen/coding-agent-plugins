...
/report --audience technical --data .claude/work/current/analysis.json
```
...
if [ -z "$DATA_SOURCE" ]; then
    ...
    if [ -d ".claude/work/current" ]; then
        DATA_SOURCE=$(find .claude/work/current -name "*.json" -o -name "*.csv" | head -1)
